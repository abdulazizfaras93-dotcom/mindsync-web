...
import s from '../matcha/trymatcha.module.css'
...
export default function TryClientPage() {
  ...
  const slug = String((params?.client as string) ?? '').toLowerCase()
  ...
  useEffect(() => {
    if (!slug) return
    fetch(`/api/try-agent?slug=${encodeURIComponent(slug)}`)
      .then(r => r.json())
      .then(d => { setFound(!!d.found); setName(d.name || '') })
  ...
  const send = async (text: string) => {
    ...
    setBusy(true)
    try {
      const r = await fetch('/api/try-agent', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ message: t, slug, sessionId: sid.current }),
      })
  ...
  if (found === false) {
    return (
      <div className={s.page}>
        <div className={s.body}><div className={s.empty}>ما حصلنا هذا العرض 🌸</div></div>
      </div>
    )
  }
  ...
  return (
    <div className={s.page}>
      <div className={s.head}>
        <div className={s.av}>
          {avOk
            ? <img src={`/clients/${slug}.jpg`} alt={display} onError={() => setAvOk(false)} />
            : <span style={{ display: 'flex', width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center', background: '#1C5038', color: '#E3B867', fontWeight: 700, fontSize: 18 }}>{initial}</span>}
        </div>
        <div className={s.hmeta}>
          <div className={s.nm}>{display}</div>
